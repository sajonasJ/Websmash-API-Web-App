...
const INTRSTNG = 'https://www.flickr.com/services/rest/?method=flickr.interestingness.getList&per_page=10&format=json&nojsoncallback=1&sort=interestingness-desc&api_key=' + API_KEY;
const GETSIZES = 'https://www.flickr.com/services/rest/?method=flickr.photos.getSizes&format=json&nojsoncallback=1&sort=interestingness-desc&api_key=' + API_KEY + '&photo_id=';
let mrequest = 0;
let mrecieved = 0;
let ready_cb;
...
function getSizes(photoObj) {
    ...
    $.get(getSizeReq, function (data) {
        let imgSize = data.sizes.size
        mrecieved++;
        photoObj.file = imgSize[3].source;
        photoObj.full = imgSize[imgSize.length - 1].source;
